...
class GipsyInterpreter:
    ...
    def handle_use(self, line):
        ...
        if len(parts) == 2:
            lib = parts[1]
            try:
                self.global_env.set(lib, __import__(lib))
            except ImportError:
...
if __name__ == "__main__":
    ...
    print("🔁 Gipsy v0.3 – REPL completo con comentarios, typeof, funciones anónimas y más")
    print("Línea vacía para ejecutar bloque. Ctrl+C o 'exit' para salir.")
    buffer = []
    while True:
        try:
