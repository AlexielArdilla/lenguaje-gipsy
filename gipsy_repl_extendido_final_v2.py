...
def evaluate(self, expr):
        ...
        try:
            # Si es una variable directa
            if expr in self.env.variables:
                return self.env.get(expr)

            # Si es una llamada a función Gipsy (ej: cuadrado(x))
            if "(" in expr and expr.endswith(")"):
                ...
                if func:
                    ...
                    try:
                        ...
                        return ret.value
                    return None  # si no hubo return explícito

            # Evaluar expresiones normales con variables
            local_env = {}
            current = self.env
            while current:
