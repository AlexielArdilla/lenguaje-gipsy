...
class GipsyInterpreter:
    ...
    def handle_use(self, line):
        ...
        if expr.startswith("fun(") and "return" in expr and expr.endswith("end"):
            header, body = expr.split(")", 1)
            args = header[4:].strip()  # lo que está dentro de fun(...)
            ret = body.strip().replace("return", "").replace("end", "").strip()
            return eval(f"lambda {args}: {ret}", {}, {})

        try:
                self.env.set(lib, __import__(lib))
            except ImportError:
                print(f"No se pudo importar la librería: {lib}")
    ...
    def execute_line(self):
        if self.lines[self.pos].strip().startswith("Use "):
            self.handle_use(self.lines[self.pos].strip())
            self.pos += 1
            return
        ...
        if line.startswith("Say "):
            ...
            self.evaluate(line)  # ejecutar métodos como lista.append(9)
  # ejecutar llamada a función
        self.pos += 1
...
if __name__ == "__main__":
    ...
    print("🔁 REPL de Gipsy v0.2 – Funcional y corregido")
