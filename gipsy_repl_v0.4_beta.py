# Gipsy v0.4-beta REPL
# Features: Say, Ask, typeof, variables, Const, function/end, return,
# if/else/end, while/end, loop N times/end, include "file", Use <mod> [as alias],
# match/case, try/catch, simple call stack guard y trazas en español.
...
class Interpreter:
    ...
    # ---------- Parser (línea a línea con bloques por 'end') ----------
    def parse(self, lines):
        tokens = [l.rstrip("\n") for l in lines]
        i = 0

        def parse_until(stops):
            nonlocal i
            block = []
            while i < len(tokens):
                line = tokens[i].strip()
                # ¿toca parar?
                if line in stops or any(line.startswith(s) for s in stops if s.endswith(" ")):
                    break
                i += 1
                if not line:
                    continue
                ...
                if line.startswith("function "):
                    ...
                    cond = line[3:].strip()
                    then_body = parse_until({"else", "end"})
                    else_body = []
                    if i < len(tokens) and tokens[i].strip().startswith("else"):
                        i += 1
                        else_body = parse_until({"end"})
                    if i < len(tokens) and tokens[i].strip() == "end":
                        i += 1
                    ...
                    try_body = parse_until({"catch", "end"})
                    catch_var = None
                    catch_body = []
                    if i < len(tokens) and tokens[i].strip().startswith("catch"):
                        m = re.match(r"catch(?:\s+([A-Za-z_]\w*))?$", tokens[i].strip())
                        if not m:
                            raise GipsyError("Sintaxis de catch inválida. Usa: catch [err]")
                        catch_var = m.group(1)
                        i += 1  # consume catch
                        catch_body = parse_until({"end"})
                    if i < len(tokens) and tokens[i].strip() == "end":
                        i += 1
                    block.append(("try", try_body, catch_var, catch_body))
                    ...
                    while i < len(tokens):
                        ...
                        if ln.startswith("case "):
                            pat = ln[5:].strip()
                            i += 1  # consume 'case ...'
                            body = parse_until({"case ", "else", "end"})
                        ...
                        if ln.startswith("else"):
                            ...
                            if i < len(tokens) and tokens[i].strip() == "end":
                                i += 1
                            break
                        raise GipsyError(f"Sintaxis de match inválida cerca de: '{ln}'")
                    block.append(("match", subject, cases, else_body))
    ...
    def run_block(self, ast, env):
        for node in ast:
            ...
            if kind == "funcdef":
                ...
                _, try_body, catch_var, catch_body = node
                try:
                    self.run_block(try_body, env)
                except Exception as _e:
                    local = Environment(parent=env)
                    if catch_var:
                        local.set(catch_var, _e)
                    self.run_block(catch_body, local)

            elif kind == "match":
                _, subject, cases, else_body = node
                subj_val = self.safe_eval(subject, env)
                matched = False
                for pat, body in cases:
                    parts = [p.strip() for p in pat.split(" or ")]
                    hit = False
                    for p in parts:
                        val = self.safe_eval(p, env)
                        if subj_val == val:
                            hit = True
                            break
                    if hit:
    ...
    def exec_stmt(self, line, env, lineno):
        # type alias <Name> = <expr>  (se guarda como string, sin enforcement aún)
        if line.startswith("type alias "):
            rest = line[len("type alias "):]
            if "=" not in rest:
                raise GipsyError("Uso: type alias Name = expr")
            name, expr = rest.split("=", 1)
            name = name.strip()
            expr = expr.strip()
            aliases = self.global_env.vars.get("__type_aliases__", {})
            if not isinstance(aliases, dict):
                aliases = {}
            aliases[name] = expr
            self.global_env.set("__type_aliases__", aliases)
            return
        ...
        if line.startswith("include "):
            ...
            if not m:
                raise GipsyError('Uso: include "archivo.gipsy"')
            path = m.group(1)
            try:
        ...
        if line.startswith("Say "):
            expr = line[len("Say "):]
            # soporte directo para typeof X en Say
            if expr.strip().startswith("typeof "):
                target = expr.strip()[len("typeof "):].strip()
                val = type(self.safe_eval(target, env)).__name__
            else:
                val = self.safe_eval(expr, env)
            print(val)
            return

        # x = Ask "..." [as number]  /  Ask "..." [as number]
...
def repl():
    print("Gipsy v0.4-beta – escribe 'exit' para salir. Finaliza un bloque con 'end'.")
    intr = Interpreter()
    buf = []
    while True:
        try:
